...
/**
 * 조회 기간(계약일 구간)과, 그 구간을 덮기 위해 호출해야 하는
 * 국토교통부 API 의 `DEAL_YMD`(YYYYMM) 목록을 계산한다.
 *
 * 실거래가 API 는 **계약월 단위**로만 조회되므로, 구간이 달을 걸치면
 * 해당 월을 모두 받아온 뒤 계약일 기준으로 다시 걸러야 한다.
 * 5년 구간이면 61개월치를 받아야 하므로, 호출량은 [TradeQueryPlan] 으로 미리 따져 본다.
 */
object TradeDateWindow {
    ...
    /** 프리셋 기간의 계약일 구간. */
    fun range(period: TradePeriod, today: LocalDate): ClosedRange<LocalDate> =
        period.range(today)
    ...
    /** 프리셋 기간을 덮는 `DEAL_YMD` 목록. */
    fun dealYmdCodes(period: TradePeriod, today: LocalDate): List<String> =
        dealYmdCodes(period.range(today))

    /**
     * 구간이 걸치는 개월 수. 그대로 지역당 API 호출 횟수가 된다.
     * 예) 최근 5년 → 61
     */
    fun monthCount(range: ClosedRange<LocalDate>): Int =
        ChronoUnit.MONTHS.between(
            YearMonth.from(range.start),
            YearMonth.from(range.endInclusive),
        ).toInt() + 1
