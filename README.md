...
`keystore/debug.keystore` 는 커밋되어 있다. AGP 는 기본적으로 빌드하는 기기마다
디버그 키를 새로 만드는데, CI 러너는 매번 새로 뜨므로 빌드할 때마다 서명이 달라진다.
그러면 기존에 깔린 앱 위에 덮어쓰기 설치가 되지 않아, 업데이트할 때마다 앱을 지우고
다시 깔아야 하고 저장해 둔 인증키도 함께 사라진다.

키를 고정하면 모든 빌드가 같은 서명을 갖는다. 디버그 전용 키이고 비밀번호도 공개된
관례값(`android`)이라 저장소에 두어도 된다. **스토어 배포용 릴리스 키는 반드시 따로
만들고 Secrets 로만 다뤄야 한다.**
...
│  ├─ search/        아파트 검색 — 받아온 자료 안에서 단지 찾기
│  └─ navigation/    화면 경로
├─ di/               Hilt 모듈
└─ ui/
   ├─ theme/         Pretendard 타이포그래피 · 컬러 · 여백 · 모서리 · Material3 테마
   └─ components/    화면이 공유하는 조각 — 카드 · 알약 칩 · 세그먼트 탭
```

## 메인 화면 (Step 3)

- **3-Way 탭** 매매 / 전세 / 월세. 월세는 보증금과 월세액을 나눠 표기한다.
- **기간 선택** 2주(기본) ~ 5년. 지역 · 평형대(20평대/30평대/40평대…) · 정렬(최신순/금액순) 필터.
- **지역은 직접 고른다** 처음에는 아무 지역도 선택되어 있지 않다. 시트에서 고른 뒤
  하단 **확인** 을 눌러야 조회가 시작된다. 칩을 누를 때마다 조회하면 열 곳을 고르는 동안
  아홉 번을 헛부르게 되고, 그게 실기기에서 `HTTP 429` 로 이어졌다.
...
- **요청 속도 조절** 전송 계층에서 요청 간격을 지키고, `429` 를 받으면 간격을 늘렸다가
  다시 시도한다(서버가 준 `Retry-After` 우선). 그래도 실패하면 사유를 화면에 그대로 남긴다.
...
```
TradeQueryPlan  →  캐시 만료 판정  →  국토부 API (동시 2 · 간격 조절)  →  파싱  →  Room 저장
...
- 상단 돋보기 아이콘에서 연다. 단지명을 **두 글자 이상** 치면 목록이 뜨고,
  하나를 골라 **조회** 를 누르면 상세로 간다.
- 상세는 (단지 + 평형) 단위라, **가장 최근에 거래된 평형**으로 연다.
  다른 평형은 상세 화면의 평형 칩에서 바꾼다.
- **검색 범위는 이미 받아온 자료다.** 실거래가 API 는 (시군구 × 계약월) 로만 응답하고
  단지명 조회를 제공하지 않아 전국 단지 목록을 미리 가질 수 없다. 그래서 찾지 못했을 때
  "없는 단지" 라고 하지 않고, 메인 화면에서 지역·기간을 골라 조회한 뒤 다시 검색하도록 안내한다.
- 매매가 없고 전세만 있는 단지도 검색된다 (두 표를 합쳐 찾는다).
...
- [x] 아파트 검색, 지역 선택 확인 버튼, 요청 속도 조절(429 대응), 디자인 정비
